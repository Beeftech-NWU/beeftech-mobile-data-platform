...
interface FarmerDao {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertFarmerAddress(address: FarmerAddressEntity)

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertFarmerRole(role: FarmerRoleEntity)

    @Query("SELECT * FROM farmers WHERE farmer_id = :farmerId")
    suspend fun getFarmerById(farmerId: String): FarmerEntity?

    @Query("SELECT * FROM farmers")
    suspend fun getAllFarmers(): List<FarmerEntity>

    @Query("SELECT * FROM farmer_addresses WHERE farmer_id = :farmerId")
    suspend fun getAddressesForFarmer(farmerId: String): List<FarmerAddressEntity>
}